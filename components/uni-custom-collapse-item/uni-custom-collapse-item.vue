	...
	<view
		...
		<view :class="{ 'uni-collapse-cell--disabled': disabled}" class="uni-collapse-cell__title" @click="onClick">
			...
			<slot name="custom-head">
				<!-- 默认插槽 -->
				<uni-icons  type='smallcircle-filled'  size="25" />
			</slot>

			<text v-if="title != null && title != '' && title != undefined "
				class="uni-collapse-cell__title-text">{{ title }}</text>

			<view  v-if="content != null && content != '' && content != undefined " class="example-body">
				<text style="font-size:13px;" >{{content}}</text>
			</view>
...
<script>
	...
	export default {
		...
		onLoad() {
			var context = uni.createCanvasContext('lineCanvas')
